...
namespace MarketHub.Gateway.Controllers.IdentityService
{
    [ApiController]
    [Route("api/auth")]
    public class AuthController : ControllerBase
    {
        ...
        /// این متد از تکرار کد جلوگیری می‌کند.
        /// </summary>
        ...
        public async Task<IActionResult> Register([FromBody] object registerRequest)
        {
            return await ForwardRequest(
                () => {
                    ...
                    return client.PostAsJsonAsync("https://localhost:7053/api/auth/register", registerRequest);
                },
                "Registration"
            );
        }
        ...
        public async Task<IActionResult> Login([FromBody] object loginRequest)
        {
            return await ForwardRequest(
                () => {
                    var client = _httpClientFactory.CreateClient();
                    return client.PostAsJsonAsync("https://localhost:7053/api/auth/login", loginRequest);
                },
                "Login"
            );
        }
        ...
        public async Task<IActionResult> Logout()
        {
            return await ForwardRequest(
                () => {
                    ...
                    return client.PostAsync("https://localhost:7053/api/auth/logout", null);
                },
                "Logout"
            );
        }
        ...
        public async Task<IActionResult> RefreshToken([FromBody] object refreshTokenRequest)
        {
            return await ForwardRequest(
                () => {
                    var client = _httpClientFactory.CreateClient();
                    return client.PostAsJsonAsync("https://localhost:7053/api/auth/refresh-token", refreshTokenRequest);
                },
                "Token refresh"
            );
        }
