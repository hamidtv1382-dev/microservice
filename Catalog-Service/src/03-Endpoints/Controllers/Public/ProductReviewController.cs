...
namespace Catalog_Service.src._03_Endpoints.Controllers.Public
{
    ...
    public class ProductReviewController : ControllerBase
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            // ارسال null به عنوان vendorUserId چون این کاربر Customer است
            var product = await _productService.GetByIdAsync(request.ProductId, null, cancellationToken);
...
if (string.IsNullOrWhiteSpace(userId))
    throw new global::Catalog_Service.src.CrossCutting.Exceptions.UnauthorizedAccessException(
        ...
        public async Task<ActionResult<PagedResponse<ProductReviewResponse>>> GetProductReviews(
            ...
            CancellationToken cancellationToken)
        {
            // ارسال null به عنوان vendorUserId
            var product = await _productService.GetByIdAsync(productId, null, cancellationToken);
            if (product == null || product.Status != ProductStatus.Published)
