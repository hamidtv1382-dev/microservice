    ...
    public class ProductVariantRepository : IProductVariantRepository
    {
        ...
        public async Task<ProductVariant> GetByIdAsync(int id, CancellationToken cancellationToken = default)
        {
            return await _dbContext.ProductVariants
                ...
                .FirstOrDefaultAsync(pv => pv.Id == id && !pv.IsDeleted, cancellationToken);
        }
        ...
        public async Task<ProductVariant> GetBySkuAsync(string sku, CancellationToken cancellationToken = default)
        {
            return await _dbContext.ProductVariants
                .Include(pv => pv.Product)
                .FirstOrDefaultAsync(pv => pv.Sku == sku && !pv.IsDeleted, cancellationToken);
        }
       ...
       CancellationToken cancellationToken = default)
        ...
        public async Task<(IEnumerable<ProductVariant> Variants, int TotalCount)> GetPagedAsync(
            ...
            CancellationToken cancellationToken = default)
        {
            var query = _dbContext.ProductVariants.Where(pv => !pv.IsDeleted);
        ...
        {
            return await _dbContext.ProductVariants
                .Where(pv => pv.ProductId == productId && pv.StockStatus == StockStatus.OutOfStock && !pv.IsDeleted)
                .ToListAsync(cancellationToken);
        }
        ...
        {
            return await _dbContext.ProductVariants
                .Where(pv => pv.ProductId == productId && pv.StockQuantity <= threshold && pv.StockQuantity > 0 && !pv.IsDeleted)
                .ToListAsync(cancellationToken);
        }
            ...
            CancellationToken cancellationToken = default)
        {
            return await _dbContext.ProductVariants
                .Where(pv => pv.ProductId == productId &&
                           ...
                           pv.Price.Amount <= maxPrice && !pv.IsDeleted)
                .ToListAsync(cancellationToken);
        }
      ...
      CancellationToken cancellationToken = default)
        ...
        {
            return await _dbContext.ProductVariants.AnyAsync(pv => pv.Id == id && !pv.IsDeleted, cancellationToken);
        }

        public async Task<bool> ExistsBySkuAsync(string sku, CancellationToken cancellationToken = default)
        {
            return await _dbContext.ProductVariants.AnyAsync(pv => pv.Sku == sku && !pv.IsDeleted, cancellationToken);
        }

        public async Task<bool> IsUniqueSkuAsync(string sku, int? excludeVariantId = null, CancellationToken cancellationToken = default)
        {
            var query = _dbContext.ProductVariants.Where(pv => pv.Sku == sku && !pv.IsDeleted);
        ...
        {
            return await _dbContext.ProductVariants.CountAsync(pv => !pv.IsDeleted, cancellationToken);
        }

        public async Task<int> CountByProductIdAsync(int productId, CancellationToken cancellationToken = default)
        {
            return await _dbContext.ProductVariants.CountAsync(pv => pv.ProductId == productId && !pv.IsDeleted, cancellationToken);
        }

        public async Task<int> CountActiveByProductIdAsync(int productId, CancellationToken cancellationToken = default)
        {
            return await _dbContext.ProductVariants.CountAsync(pv => pv.ProductId == productId && pv.IsActive && !pv.IsDeleted, cancellationToken);
        }
        ...
        public async Task<IEnumerable<ProductAttribute>> GetAttributesAsync(int variantId, CancellationToken cancellationToken = default)
        {
            return await _dbContext.ProductAttributes
                .Where(pa => pa.ProductVariantId == variantId && !pa.IsDeleted)
                .ToListAsync(cancellationToken);
        }
        ...
        public async Task<IEnumerable<ImageResource>> GetImagesAsync(int variantId, CancellationToken cancellationToken = default)
        {
            return await _dbContext.ImageResources
                .Where(i => EF.Property<int?>(i, "ProductVariantId") == variantId && !i.IsDeleted)
